...
def register_user(
    ...
    background_tasks.add_task(send_welcome_email, str(new_user.username), str(new_user.email))
...
def user_update(
    ...
    if current_user.id != user_id:
        raise  HTTPException(
            ...
            detail="Can not update another user's account"
        )
...
def delete_user( 
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Can not delete another user's account"
        )
